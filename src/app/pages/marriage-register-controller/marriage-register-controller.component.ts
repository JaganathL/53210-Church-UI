...
@Component({
    selector: 'app-marriage-register-controller',
    standalone: true,
    imports: [CommonModule, FormsModule, ReactiveFormsModule],
    templateUrl: './marriage-register-controller.component.html',
    styleUrls: ['./marriage-register-controller.component.scss']
})
export class MarriageRegisterControllerComponent implements OnInit, OnDestroy {
    searchQuery = '';
    showModal = false;
    isEditMode = false;
    ...
    loadMarriageRecords(): void {
        this.marriageRegisterService.getAllMarriageRecords()
            ...
            .subscribe({
                next: (response: any) => {
                    this.marriageRecords = response.content || [];
                    this.updateFilteredRecords();
                },
                error: (error: any) => {
    ...
    updateFilteredRecords(): void {
        if (!this.searchQuery.trim()) {
            this.filteredRecordsArray = [...this.marriageRecords];
        } else {
            const query = this.searchQuery.toLowerCase();
            this.filteredRecordsArray = this.marriageRecords.filter(record =>
    ...
    onEdit(record: MarriageRecord): void {
        ...
        this.marriageForm.patchValue({
            ...
            witness1Name: record.witnesses?.witness1 || '',
            witness2Name: record.witnesses?.witness2 || '',
            witness3Name: record.witnesses?.witness3 || ''
        });
    }
    ...
    onSaveRecord(): void {
        if (this.marriageForm.valid) {
            const witnesses: any = {
                witness1: this.marriageForm.get('witness1Name')?.value
            };

            if (this.marriageForm.get('witness2Name')?.value) {
                witnesses.witness2 = this.marriageForm.get('witness2Name')?.value;
            }

            if (this.marriageForm.get('witness3Name')?.value) {
                witnesses.witness3 = this.marriageForm.get('witness3Name')?.value;
            }
